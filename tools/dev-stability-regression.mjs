...
await test('Competition live controls follow the unified programme, results and Return Home contract',()=>{
 const f=fixture();try{
  ...
  const e={id:'meet',week:5,results:{},completed:false};
  ...
  assert.equal(JSON.stringify(f.w.opened[2]),'["meet","results"]');
  f.w.document.getElementById('v3day').click();
  assert.equal(JSON.stringify(f.w.opened[3]),'["meet","home"]');
 }finally{f.dom.window.close()}
});
