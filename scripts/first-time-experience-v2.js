...
function trainingDecisionSignature(){
 const rows=managed().filter(a=>!a.retired).map(a=>{
  if(window.AMTrainingSystem2){const t=a.attributeDevelopment;return t?[String(a.id),{focus:t.focus,intensity:t.intensity}]:null}
...
function phaseCopy(){
 ...
 if(phase==='training')return{eyebrow:'WEEK 2 • TRAINING',title:'Review one athlete’s training',body:`${coach.name}: “${rec.reason}” Open Training and make one deliberate call: keep the current plan, or adjust it if the evidence gives you a reason.`,action:'REVIEW TRAINING',route:'training',required:true};
 if(phase==='scouting')return{eyebrow:'WEEK 3 • SCOUTING',title:'Set your first scouting assignment',body:`${scout.name}: “Open Scouting, choose the search focus and confirm where you want the network looking.”`,action:'OPEN SCOUTING',route:'scouting',required:true};
 ...
 if(phase==='competition')return{eyebrow:'COMPETITION WEEK',title:e?.name||'Competition day',body:'Open the meeting from the schedule, then choose which event you want to watch.',action:'OPEN COMPETITION',route:'competition'};
...
function handleClick(ev){
 ...
 if(b.hasAttribute('data-ftx-keep-training')){markStep('trainingDecision');saveSafe();toastSafe('Training reviewed — current plan kept');try{if(typeof drawTraining==='function')drawTraining()}catch(_){ }scheduleDecorate();return}
