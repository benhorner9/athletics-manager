/* ===== Continuous Track Race Overhaul V2 ===== */
(function(){
'use strict';

const TRACK_RACE_VERSION=2;
  ...
  return raw.map((stage,si)=>{
    const rows=(stage.rows||[]).map((r,i)=>{
      ...
      return {
        ...
        flag:typeof flag==='function'?flag(r.nation||a?.nation||source.nation):'',
...
function stageRaceState(stage,d,local){
  const distance=raceDistance(d), rows=stage.rows, maxGap=Math.max(0,...rows.map(r=>officialGapMetres(r,rows,distance))), runoff=Math.min(10,Math.max(3,maxGap+2));
  const leaderMetres=clamp01(local)*(distance+runoff), t=clamp01(leaderMetres/distance);
  const items=rows.map(row=>{
    const official=officialGapMetres(row,rows,distance), finishPull=official*Math.pow(t,2.35), shift=styleShift(row,stage,distance,t);
    const metres=Math.max(0,Math.min(distance+runoff,leaderMetres-finishPull+shift));
  ...
  return {distance,leaderMetres,t,items,order:items.map(x=>x.row)};
}
...
function updateTrackVisual(e,d,stage,state,local){
  ...
  const leaderMetres=Math.max(0,Math.min(distance,Math.max(...state.items.map(x=>x.metres))));
  const distanceEl=root.querySelector('[data-track-distance]'), orderEl=root.querySelector('[data-track-order]'), clockEl=root.querySelector('[data-track-clock]');
...
function runTrackPlayback(e,d,results,career){
  ...
  function frame(now){
    ...
    while(track.nextCheckpoint<stage.checkpoints.length&&leaderMetres>=stage.checkpoints[track.nextCheckpoint]){
      const metres=stage.checkpoints[track.nextCheckpoint++], cpState=stageRaceState(stage,d,metres>=raceDistance(d)?1:local);
      emitHighlight(e,d,stage,metres,cpState,track);
    }
    if(local>=1){
...
/* ===== End Continuous Track Race Overhaul V2 ===== */