/* Athletics Manager — 4x100m Relay V1
   ...
   - Nation/team records and a dedicated live relay presentation.
...
function teamObject(nation,d,lineup=null){
...
function relayField(e,d){
 ...
 rivals.forEach(x=>teams.push(teamObject(x.n,d,null)));return teams.slice(0,8);
}
...
function renderRelayDiscipline(e,d){
 ...
 const cont=root.querySelector('[data-relay-continue]');if(cont)cont.onclick=()=>{const ds=(e.disc||[]).filter(x=>x&&x!=='ALL');if(ds.length&&ds.every(x=>Array.isArray(e.results?.[x]))&&!e.completed){activeEventDisc=null;competitionMode='overview';safe(()=>finaliseEvent(e,false),null);return}activeEventDisc=null;competitionMode='overview';drawCompetition()};
