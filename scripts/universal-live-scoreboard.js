...
function boardShell(kicker,title,status,rightHead,body,note){return `<div class="universal-live-board"><header class="uls-head"><div><small>${esc(kicker)}</small><strong>${esc(title)}</strong></div><span>${esc(status)}</span></header><div class="uls-cols"><span>POS</span><span>ATHLETE</span><span>${esc(rightHead)}</span></div><div class="uls-list">${body}</div><footer class="uls-foot">${esc(note)}</footer></div>`}
...
function recordRaceSplits(stage,state){
  if(!stage||!state?.items?.length)return;if(stage.key!==raceStageKey)resetRaceSplits(stage.key);
  ...
  for(const item of state.items){
    ...
    for(const cp of checkpoints){
      if(store.has(cp)||metres<cp)continue;let crossing=time;
...
function renderTrack(board,e,d,track){
  const stage=track.stages?.[track.stageIndex]||track.stages?.[0],state=track.state;if(!stage||!state?.order?.length)return;recordRaceSplits(stage,state);
  ...
  const body=order.map((row,i)=>{const pos=i+1,item=state.items.find(x=>x.row.id===row.id),split=latestSplitFor(row.id),finished=Number(item?.metres)>=distance,primary=finished&&Number.isFinite(Number(row.perf))?formatPerf(d,row.perf):split?formatSplit(split.time):'—',secondary=finished?'FINISH':split?`${split.metres}m split`:`${Math.round(Number(item?.metres)||0)}m`;return `<div class="uls-row ${row.nation===managed()?'managed':''}${changedClass(row.id,pos)}"><b class="uls-pos">${pos}</b><i class="uls-dot" style="background:${dotColour(row)}"></i><div class="uls-name"><strong>${esc(row.name)}</strong><small>${esc(nation(row))}${row.lane?` • Lane ${row.lane}`:''}</small></div><div class="uls-value"><strong>${esc(primary)}</strong><small>${esc(secondary)}</small></div></div>`}).join('');
...
function attemptState(e,d){
  ...
  for(let i=0;i<=idx;i++){
    const cue=cues[lines[i]];if(!cue?.athleteId||!states.has(cue.athleteId)||!Number.isFinite(Number(cue.attempt))||cue.family==='height'||!('mark' in cue))continue;
    const st=states.get(cue.athleteId),a=Math.max(0,Number(cue.attempt)-1);st.attempts[a]=Number.isFinite(cue.mark)?Number(cue.mark):null;
  }
...
function heightState(e,d){
  ...
  for(let i=0;i<=idx;i++){
    const cue=cues[lines[i]];if(cue?.family!=='height'||!cue.athleteId||!states.has(cue.athleteId))continue;const st=states.get(cue.athleteId),height=Number(cue.height),attempt=Number(cue.attempt)||1;
...
function sync(){
  if(!disciplineRunning||!liveEventView?.event||!liveEventView?.disc)return;const board=document.getElementById('liveScoreboard');if(!board)return;const e=currentEvent(),d=currentDisc(),track=liveEventView?.trackOverhaulV4;
  if(track?.state?.order?.length){renderTrack(board,e,d,track);return}
  if(typeOf(d)==='height'){renderHeight(board,e,d);return}
  if(typeOf(d)!=='time'){renderAttempts(board,e,d)}
}

installStyles();window.setInterval(sync,POLL_MS);
if(typeof UPDATES!=='undefined'&&!UPDATES.some(u=>u.title==='Universal Live Event Scoreboard'))UPDATES.unshift({date:'9 September 2026',title:'Universal Live Event Scoreboard',items:['The right-hand Event Day scoreboard is now the authoritative live standings panel across event types.','Track races update position continuously from the same race state as the on-track athletes, with an individual split time recorded for every athlete at each checkpoint they cross.','Throws and distance-attempt events recalculate the order immediately after every attempt, including fouls and the full completed attempt series.','High Jump standings update after every clearance, miss or pass using current best height and countback, without revealing future attempts.','The duplicate in-arena race leaderboard has been removed so Event Day has one clear live scoreboard on the right.']});
