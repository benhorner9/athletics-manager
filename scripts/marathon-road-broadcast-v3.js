...
const EVENT_TYPES=new Set(['START','ATTACK','LEAD_CHANGE','PACK_SPLIT','ATHLETE_DROPPED','CHASE_STARTED','GAP_CLOSED','INJURY','MAJOR_FADE','CHECKPOINT','FINAL_KM','FINISH']);
...
const ownNation=()=>{try{return typeof managedNation==='function'?managedNation():s?.managedNation||'GREAT BRITAIN'}catch(_){return s?.managedNation||'GREAT BRITAIN'}};
...
function buildRaceEvents(e,d,results){
 ...
 for(let i=1;i<xs.length;i++){
  ...
  if(a.leaderId&&b.leaderId&&a.leaderId!==b.leaderId)addEvent(events,{type:'LEAD_CHANGE',metres:mid,athleteId:b.leaderId,priority:8,reason:'leader changed'},e,d);
  if(a.leadPackSize-b.leadPackSize>=2)addEvent(events,{type:'PACK_SPLIT',metres:mid,priority:7,reason:`lead pack ${a.leadPackSize}→${b.leadPackSize}`},e,d);
  const a2=secondGap(a),b2=secondGap(b);
  if(b2>=6&&b2-a2>=4)addEvent(events,{type:'ATTACK',metres:mid,athleteId:b.leaderId,priority:8,reason:'front gap opened'},e,d);
  if(a2>=7&&a2-b2>=3)addEvent(events,{type:b2<=2?'GAP_CLOSED':'CHASE_STARTED',metres:mid,athleteId:b.entries[1]?.id||null,priority:b2<=2?8:7,reason:'leader gap reduced'},e,d);
  let biggest=null;
  for(const x of a.entries.slice(0,18)){
   const rb=rankOf(b,x.id),drop=rb-x.rank,gain=Number(b.gaps[x.id]||0)-Number(a.gaps[x.id]||0);if(drop>=4||gain>=15){const score=drop*3+gain+(own.has(x.id)?20:0);if(!biggest||score>biggest.score)biggest={id:x.id,drop,gain,score}}
  }
  if(biggest)addEvent(events,{type:'MAJOR_FADE',metres:mid,athleteId:biggest.id,priority:own.has(biggest.id)?9:6,reason:'position/gap deterioration'},e,d);
  for(const id of own){const ra=rankOf(a,id),rb=rankOf(b,id);if(ra<=a.leadPackSize&&rb>b.leadPackSize+1)addEvent(events,{type:'ATHLETE_DROPPED',metres:mid,athleteId:id,priority:9,reason:'programme athlete lost lead pack'},e,d);else if(ra-rb>=4)addEvent(events,{type:'CHASE_STARTED',metres:mid,athleteId:id,priority:8,reason:'programme athlete advanced'},e,d)}
 }
...
function coverageTarget(e,events,results){
 const major=isMajor(e),premium=Number(e?.star||0)>=5,finishGap=Math.max(0,Number(results[1]?.perf||0)-Number(results[0]?.perf||0)),dynamic=events.filter(x=>!['START','CHECKPOINT','FINAL_KM','FINISH'].includes(x.type)).length;
 let target=major?300000:premium?215000:180000;target+=Math.min(major?50000:55000,dynamic*(major?5500:7000));if(finishGap<=10)target+=major?18000:20000;return clamp(target,180000,360000)
}
function commentaryFor(h,e,d,results){
 ...
 if(h.type==='START')return `GAVIN POTTS • START\n${e.location}. ${course(e).profile}. The field is away over 42.195 kilometres; the opening section is about position and restraint.`;
 ...
 if(h.type==='FINISH'){const winner=results[0],runner=results[1];return `FINISH • 42.195 KM\nGavin Potts — ${winner?`${winner.name} wins in ${raceClock(d,winner.perf)}`:'The winner reaches the line'}${runner?`. ${runner.name} follows ${fmtGap(Number(runner.perf)-Number(winner.perf))}`:''}. The important finishers are coming home now.`}
...
function director(e,d,results){
 const events=buildRaceEvents(e,d,results),major=isMajor(e),standards=major?[0,2000,5000,10000,15000,20000,21097.5,25000,30000,32500,35000,36500,38000,39000,40000,40500,41195,41400,41695,DIST]:[0,2000,5000,10000,21097.5,25000,30000,35000,38000,40000,41195,41695,DIST],moments=[];
 for(const metres of standards){const type=metres===0?'START':metres===41195?'FINAL_KM':metres===DIST?'FINISH':'CHECKPOINT';moments.push({type,metres,priority:type==='CHECKPOINT'?5:10,standard:true})}
 const dynamic=events.filter(x=>!['START','CHECKPOINT','FINAL_KM','FINISH'].includes(x.type)).sort((a,b)=>b.priority-a.priority);
 for(const ev of dynamic){if(!moments.some(x=>Math.abs(x.metres-ev.metres)<(major?350:550)))moments.push({...ev,standard:false});else{const near=moments.reduce((best,x)=>Math.abs(x.metres-ev.metres)<Math.abs(best.metres-ev.metres)?x:best,moments[0]);if(ev.priority>near.priority){near.type=ev.type;near.athleteId=ev.athleteId;near.priority=ev.priority;near.reason=ev.reason}}}
 moments.sort((a,b)=>a.metres-b.metres||b.priority-a.priority);
 const dedup=[];for(const m of moments){if(dedup.length&&Math.abs(dedup.at(-1).metres-m.metres)<180&&m.type!=='FINISH')continue;dedup.push(m)}
 const target=coverageTarget(e,events,results),weights=dedup.map(m=>m.type==='FINISH'?1.65:m.type==='FINAL_KM'?1.45:m.type==='START'?1.35:['ATTACK','LEAD_CHANGE','PACK_SPLIT','ATHLETE_DROPPED','CHASE_STARTED','GAP_CLOSED','MAJOR_FADE','INJURY'].includes(m.type)?1.25:1),unit=target/weights.reduce((a,b)=>a+b,0);
 const highlights=dedup.map((m,i)=>{
  const next=dedup[i+1]?.metres??DIST,prev=dedup[i-1]?.metres??0,scene=sceneAt(e,m.metres),phase=m.type==='START'?'start':m.type==='FINISH'?'finish':m.type==='FINAL_KM'?'final':'race';
  let from=m.type==='START'?0:Math.max(prev,m.metres-(m.metres>=35000?420:700)),to=m.type==='FINISH'?DIST:Math.min(next,m.metres+(m.metres>=35000?520:760));if(m.type==='START')to=2000;if(m.type==='FINAL_KM'){from=40950;to=41650}
  const snapshot=snapshotAt(e,d,results,m.metres),camera=m.type==='ATTACK'||m.type==='LEAD_CHANGE'?'breakaway':m.athleteId&&(e.entries?.[d]||[]).includes(m.athleteId)?'athlete':m.type==='PACK_SPLIT'?'wide':'pack',durationMs=Math.round(clamp(unit*weights[i],7000,28000));
...
function selectDisplay(snapshot,e,d,h){
 ...
 snapshot.entries.slice(0,h.camera==='wide'?14:11).forEach(take);
 if(h.camera==='athlete'&&h.athleteId){const idx=snapshot.entries.findIndex(x=>x.id===h.athleteId);for(let i=Math.max(0,idx-2);i<=Math.min(snapshot.entries.length-1,idx+2);i++)take(snapshot.entries[i])}
 snapshot.entries.filter(x=>own.has(x.id)).forEach(take);return picked.slice(0,16)
}
...
window.AMMarathonHighlightDirector={version:VERSION,buildRaceEvents,direct:director,snapshotAt};
