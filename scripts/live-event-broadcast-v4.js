...
function commentaryHTML(c,e,d){let list=c?.comments||[];if(!c&&Array.isArray(e?.commentary?.[d]))list=e.commentary[d].map((text,i)=>({text:normaliseSpeech(text),time:i+1,big:i===e.commentary[d].length-1}));if(!c&&Array.isArray(e?.results?.[d])){const speech=officialMomentSpeech(e,d,officialRows(e,d,e.results[d]));if(speech&&list.at(-1)?.text!==speech)list=[...list,{text:speech,big:true}]}const current=list.at(-1),history=list.slice(-5,-1).reverse();return`<div class="v3comments lv4-comments"><header><div><small>LIVE COMMENTARY</small><strong>Gavin Potts</strong></div><span>${c?'ON AIR':'EVENT LOG'}</span></header><div class="lv4-current-comment">${current?`<small>GAVIN POTTS</small><p>${E(current.text)}</p>`:'<small>GAVIN POTTS</small><p>Waiting for the event to begin.</p>'}</div><div class="lv4-comment-history">${history.map(x=>`<div><time>${E(x.time||'')}</time><p>${E(x.text)}</p></div>`).join('')}</div></div>`}
