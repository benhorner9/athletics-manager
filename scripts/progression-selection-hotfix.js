/* ===== Event Progression + Urgent Selection Hotfix ===== */
(function(){
'use strict';
if(window.__amProgressionHotfix)return;window.__amProgressionHotfix=1;
const $=id=>document.getElementById(id),esc=v=>typeof profileEscape==='function'?profileEscape(String(v??'')):String(v??'');

/* Presentation must never be able to stop the simulation loop. */
if(typeof renderMatchdayCommentary==='function'){
 const baseCommentary=renderMatchdayCommentary;
 renderMatchdayCommentary=function(el,line){
  try{return baseCommentary(el,line)}catch(err){
   console.warn('Commentary render recovered',err);
   try{if(el){const p=String(line||'').split('\n');el.innerHTML=`<div class="template-commentary-line latest"><small>${esc(p.shift()||'LIVE')}</small><p>${esc(p.join(' ')||String(line||''))}</p></div>`}}catch(_){}
   return null;
  }
 }
}
...
/* Last-resort watchdog: never leave a career trapped inside a failed renderer. */
let lastSig='',lastMove=performance.now();
function playbackSig(){
 if(!disciplineRunning||!liveEventView)return '';
 if(liveEventView.trackOverhaulV4){const t=liveEventView.trackOverhaulV4;return `t:${t.stageIndex||0}:${Math.round((Number(t.local)||0)*1000)}:${liveEventView.lines?.length||0}`}
...
function enhanceInbox(){
 ...
 for(const m of s.emails||[]){const item=root.querySelector(`[data-open="${CSS.escape(String(m.id))}"]`);if(item)item.classList.toggle('am-selection-urgent',!!(m.type==='selection'&&!m.selectionSubmitted&&(m.summitRegistration?!summitLocked():!s.events.find(e=>e.id===m.eventId)?.decision)))}
}
function enhanceReader(){
 const r=$('reader'),m=(s?.emails||[]).find(x=>x.id===openMail)||(s?.emails||[]).at(-1);if(!r||!m||m.type!=='selection')return;
 r.querySelectorAll('.am-urgent-selection,.am-hotfix-confirmed').forEach(x=>x.remove());
 const head=r.querySelector('.reader-head');
 if(m.selectionSubmitted){head?.insertAdjacentHTML('afterend',confirmedHTML(m));return}
 head?.insertAdjacentHTML('afterend','<div class="am-urgent-selection"><small>URGENT · RESPONSE REQUIRED</small><strong>Selection must be confirmed before the season can advance.</strong></div>');
 const e=m.eventId?(s.events||[]).find(x=>x.id===m.eventId):null,btn=$('confirmSelection');
 if(btn&&e&&!e.completed&&!e.decision){btn.disabled=false;btn.textContent='CONFIRM & LOCK SELECTION';btn.onclick=()=>lockManualSelection(m,e)}
}
...
/* ===== End Event Progression + Urgent Selection Hotfix ===== */