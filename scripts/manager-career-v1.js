...
function careerHTML(){
 ...
 return `${seasonCardsHTML()}<div class="am-mgr-two">${jobHistoryHTML()}${contractHistoryHTML()}</div>${firstsHTML()}<section class="am-mgr-card"><div class="am-mgr-card-head"><div><small>CAREER TIMELINE</small><h2>${events.length} career moments</h2></div><div class="am-mgr-pager"><button type="button" data-manager-timeline="newer" ${timelinePage===0?'disabled':''}>NEWER</button><span>${timelinePage+1}/${pages}</span><button type="button" data-manager-timeline="older" ${timelinePage>=pages-1?'disabled':''}>OLDER</button></div></div>${page.length?`<div class="am-mgr-timeline">${page.map(timelineEntry).join('')}</div>`:emptyState('Timeline ready','Major career events will appear here as they happen.')}</section>`;
}
...
function installHooks(){
 ...
 hook('acceptCareerJob',ctx=>{const moved=ctx.before.nation!==currentNation(),mc=careerStore();if(moved){try{window.AMProgrammeEconomy?.handleNationChange?.(ctx.before.nation)}catch(err){console.warn('Programme Economy nation handover failed',err)}}captureArrival(mc,currentNation(),true);addEvent(mc,`career-job:${currentNation()}:${nowSeason()}:${safeNumber(career().cycleNumber)}`,'appointment',moved?`Joined ${nationLabel(currentNation())}`:`Renewed with ${nationLabel(currentNation())}`,moved?'A new national programme role began at the start of the Olympic cycle.':'A new four-year Olympic-cycle contract was accepted.',{nation:currentNation()});syncAll({captureArrival:true});persist()});
