...
const TRANSIENT_STATUS_MAX_AGE=45*60*1000;
...
const statusAge=iso=>{const t=Date.parse(iso||'');return Number.isFinite(t)?Math.max(0,Date.now()-t):0};
function normalisePipeline(value){
 if(!value||typeof value!=='object')return value;
 const p={...value},mode=String(p.state||'');
 if(!['running','deploying','passed'].includes(mode)||!p.updatedAt)return p;
 if(statusAge(p.updatedAt)<=TRANSIENT_STATUS_MAX_AGE)return p;
 p.originalState=mode;
 p.state='stale';
 p.stale=true;
 p.staleDetectedAt=new Date().toISOString();
 p.label='Pipeline status expired; no active test run is assumed';
 return p;
}
...
function ensureStyle(){
 ...
 #amDevBuildChip .dot{width:7px;height:7px;border-radius:50%;background:#6b8090;box-shadow:0 0 0 3px #6b80901c}#amDevBuildChip.live .dot{background:#69d39a;box-shadow:0 0 0 3px #69d39a22}#amDevBuildChip.update .dot,#amDevBuildChip.deploying .dot,#amDevBuildChip.passed .dot{background:#f0b85a;box-shadow:0 0 0 3px #f0b85a22}#amDevBuildChip.running .dot{background:#74c7e8;box-shadow:0 0 0 3px #74c7e822;animation:amDevPulse 1.2s ease-in-out infinite}#amDevBuildChip.failed .dot,#amDevBuildChip.error .dot{background:#d96d6d;box-shadow:0 0 0 3px #d96d6d22}#amDevBuildChip.stale .dot{background:#d9a85f;box-shadow:0 0 0 3px #d9a85f22}@keyframes amDevPulse{50%{transform:scale(1.45);opacity:.55}}
...
function modeLabel(mode){
 ...
 if(mode==='stale')return `DEV · STATUS EXPIRED · ${sha}`;
...
function panelStateLabel(mode){return mode==='live'?'LIVE':mode==='running'?'TESTING':mode==='passed'?'QA PASSED':mode==='deploying'?'DEPLOYING':mode==='failed'?'FAILED':mode==='stale'?'STATUS EXPIRED':mode==='update'?'RELOAD REQUIRED':mode==='error'?'STATUS UNKNOWN':'CHECKING'}
...
function renderPanel(){
 ...
 const title=mode==='running'?'A new dev build is being tested':mode==='passed'?'QA has passed':mode==='deploying'?'Dev deployment in progress':mode==='failed'?'Latest candidate failed QA':mode==='stale'?'Pipeline status has expired':mode==='update'?'A newer dev build is live':'Dev build status';
 ...
 panel.innerHTML=`<section id="amDevBuildPanel" class="am-dev-build-panel"><div class="am-dev-build-panel-head"><div><small>Development Client</small><h3>${esc(title)}</h3><p>${esc(detail)}</p></div><div class="am-dev-build-state ${mode}">${panelStateLabel(mode)}</div></div>${progressHtml(mode)}${mode==='failed'?'<p class="am-dev-pipeline-note"><strong>Not deployed.</strong> The current dev client remains on the last build that passed every gate.</p>':''}${mode==='stale'?'<p class="am-dev-pipeline-note"><strong>No test is assumed to still be running.</strong> The current dev client remains on the last confirmed deployed build.</p>':''}${mode==='passed'?'<p class="am-dev-pipeline-note"><strong>All QA gates passed.</strong> Deployment is next.</p>':''}${mode==='update'?'<div style="margin-top:10px"><button type="button" class="btn" data-am-dev-panel-reload>RELOAD DEV CLIENT</button></div>':''}${rows?`<div class="am-dev-build-history">${rows}</div>`:''}</section>`;
...
async function refresh(){
 try{
  ...
  state.server=server;state.history=Array.isArray(history)?history:[];state.pipeline=normalisePipeline(pipeline);state.status='ready';
 }catch(err){state.status='error';console.warn('[Athletics Manager] Dev build status unavailable',err)}
 renderChip();renderPanel()
}
async function start(){
 ensureStyle();claimMenuPanel();renderPanel();renderChip();window.__athleticsDevBuildStatus={version:3,state,refresh,render:()=>{renderChip();renderPanel()},normalisePipeline};
