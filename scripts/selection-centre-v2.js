...
function managedCandidates(d){return (s?.athletes||[]).filter(a=>a&&a.nation===managedNation()&&a.disc===d&&!a.retired).sort((a,b)=>(b.inSquad!==false)-(a.inSquad!==false)||abilityValue(b)-abilityValue(a)||String(a.name).localeCompare(String(b.name)))}
...
function validateNormal(e,st=ensureNormalState(e),includeWarnings=true){
 ...
 for(const d of ds){
  ...
  if(!picked.length&&available.length)errors.push(validationIssue(d,'empty',`${discLabel(d)} has no athlete selected.`));
...
function validateSummit(st=ensureSummitState(),includeWarnings=true){
 ...
 for(const d of ctx.discs){const ids=draftFor(ctx,d),available=getCandidates(ctx,d).filter(x=>x.av.ok);if(!ids.length&&available.length)errors.push(validationIssue(d,'empty',`${discLabel(d)} has no Summit athlete selected.`));if(ids.length>2)errors.push(validationIssue(d,'limit',`${discLabel(d)} exceeds the two-athlete Summit limit.`));for(const id of ids){const a=s.athletes.find(x=>x.id===id),av=a?summitAvailability(a):{ok:false};if(!av.ok)errors.push(validationIssue(d,'eligibility',`${a?.name||'Selected athlete'} is unavailable for the Summit Series.`,'error',id));if(includeWarnings&&a){const ad=summitAdvice(a,summitSeasonState().meetings?.[1]);if(ad?.tone==='warn')warnings.push(validationIssue(d,'schedule',`${a.name}: ${ad.text}`,'warn',id));if((a.fatigue||0)>=55)warnings.push(validationIssue(d,'fatigue',`${a.name} is carrying high fatigue (${a.fatigue}).`,'warn',id))}}}
 return{errors,warnings:dedupeIssues(warnings)};
}
