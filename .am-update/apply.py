...
# Activate role-aware inbox voices and cache-bust every edited player-facing source.
rep('game.html',
...
# Training: talk like a coach, not an implementation note.
for old,new in [
...
# Scouting: describe evidence and uncertainty, never hidden implementation history.
...
# Manager career: remove meta language about what the software chooses to record.
for old,new in [
...
# Selection: concise coaching judgement rather than generated sports prose.
for old,new in [
...
# Onboarding: current controls, no stale workflow or game/meta language.
rep('scripts/first-time-experience-v2.js',
    ...
    'onboarding scouting instruction')
rep('scripts/first-time-experience-v2.js',
...
# World/staff news: factual appointment copy, not PR language.
...
# Role-aware inbox: funded athlete agreements are Finance; existing saved mail gets prose cleanup
# without retroactively changing the stored sender name.
...
# Historical update-panel phrases are still player-facing; clean them at render time rather than editing game core.
...
# Build contracts: character voices must load in the correct place and known slop cannot return.
...
# Permanent writing contract for future feature work.
